    ...
    private fun loginUser() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                Log.d("Tagg", "User retrieved from repository: ${user?.toString() ?: "null"}")

                requireActivity().runOnUiThread {
                    if (user != null) {
                        Log.d("Tagg", "User found - ID: ${user.id}, EmployeeNumber: ${user.employeeNumber}, Password hash: ${user.password}")
                        if (BCrypt.checkpw(password, user.password)) {
                            ...
                            Log.d("Tagg", "Password mismatch for employeeNumber: $employeeNumber")
                        }
                    } else {
                        ...
                        Log.d("Tagg", "No user found for employeeNumber: $employeeNumber")
                    }
                }
            } catch (e: Exception) {
                requireActivity().runOnUiThread {
                    Toast.makeText(requireContext(), "Ошибка авторизации: ${e.message}", Toast.LENGTH_SHORT).show()
                    Log.e("Tagg", "Login error: ${e.message}", e)
                }
            }
        }
    }
